"""
Adapted from the passthrough FS from here: https://github.com/sholsapp/fusepy-base
...
This is an example of a simple filesystem written using Pyfuse,
the python language wrapper around libFUSE, the "filesystems in userspace" library

You'll need to modify the constructor (__init__), open(), create(), 
read(), write(), truncate(), release(), and getattr methods.  

See the assignment description for what to do
...
class EncFS(Operations):
    ...
    def __init__(self, root):
        ...
        #TODO, FINISH ME!!
        self.openedFiles = {}
        self.password = bytes(getpass.getpass("Please input password: "), 'ascii')
        self.fileCount = 3
    ...
    def getattr(self, path, fh=None):
        ...
        directory = dict((key, getattr(st, key)) for key in ('st_atime', 'st_ctime',
        ...
        if (os.path.isfile(full_path)):
            if (full_path in self.openedFiles):
                directory['st_size'] = len(self.openedFiles[full_path])
            else:
                ...
                f = Fernet(key)
                print("length: " + str(len(encryptedBytes)))
                directory['st_size'] = len(f.decrypt(encryptedBytes[16:]))
...
#        print("hello good")
        ...
        full_path  = self._full_path(path)
        if((not os.path.exists(full_path)) or (full_path in self.openedFiles)):
            return -1
        with open(full_path, "rb") as f:
            encryptedBytes = f.read()
        ...
        key = base64.urlsafe_b64encode(kdf.derive(self.password))
        f = Fernet(key)
        self.openedFiles[full_path] = f.decrypt(encryptedBytes[16:])
    ...
    def create(self, path, mode, fi=None):
        full_path = self._full_path(path)
        fileDirectory = os.open(full_path, os.O_RDWR|os.O_CREAT)
        os.close(fileDirectory)
        self.openedFiles[full_path] = bytes()
        self.fileCount = self.fileCount + 1
        return self.fileCount
    ...
    def read(self, path, length, offset, fh):
        ...
        """
        return self.openedFiles[self._full_path(path)][offset:offset+length]
    ...
    def write(self, path, buf, offset, fh):
        ...
        """
        self.openedFiles[self._full_path(path)] = buf[offset:]
        return len(buf)
    ...
    def truncate(self, path, length, fh=None):
        ...
        fileBytes = self.openedFiles[self._full_path(path)]
        if (len(fileBytes) >= length):
            fileBytes = fileBytes[:length]
        else:
            fileBytes = fileBytes + bytes([0 for i in range(length - len(fileBytes))])
   ...
   '''                                            
    ...
    def release(self, path, fh):
        ...
        f = Fernet(key)
        full_path = self._full_path(path)
        encryptedBytes = f.encrypt(bytes(self.openedFiles[full_path]))
        fd = open(full_path, 'wb')
        fd.write(salt + encryptedBytes)
        fd.close()
        return slef.openedFiles[full_path]
